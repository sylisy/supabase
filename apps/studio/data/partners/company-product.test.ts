...
import { getAccountRequest, confirmAccountRequest, PARTNER_API_PATH } from './company-product'
...
describe('company-product', () => {
  ...
  describe('PARTNER_API_PATH', () => {
    it('is derived from company and product names in lowercase', () => {
      expect(PARTNER_API_PATH).toBe('partners/company/product')
    })
  })
  ...
  describe('getAccountRequest', () => {
    it('calls the correct URL and returns account request details', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        `https://api.supabase.io/${PARTNER_API_PATH}/provisioning/account_requests/ar_123`,
    ...
    it('encodes arId to prevent path injection', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        `https://api.supabase.io/${PARTNER_API_PATH}/provisioning/account_requests/..%2Fevil`,
        expect.objectContaining({ headers: expect.any(Headers) })
      )
    })
  })
  ...
  describe('confirmAccountRequest', () => {
    it('calls the correct URL with POST and returns confirmation result', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        `https://api.supabase.io/${PARTNER_API_PATH}/provisioning/account_requests/ar_123/confirm`,
