...
/**
 * TODO(raulb): update once these are defined
 */
export const PARTNER_COMPANY_NAME = 'Company'
export const PARTNER_PRODUCT_NAME = 'Product'
export const PARTNER_API_PATH = `partners/${PARTNER_COMPANY_NAME.toLowerCase()}/${PARTNER_PRODUCT_NAME.toLowerCase()}`
...
export async function getAccountRequest(arId: string): Promise<AccountRequestDetails> {
  ...
  const response = await fetchHandler(
    `${BASE_API_URL}/${PARTNER_API_PATH}/provisioning/account_requests/${safeArId}`,
    { headers }
  )
...
export async function confirmAccountRequest(
  ...
  const response = await fetchHandler(
    `${BASE_API_URL}/${PARTNER_API_PATH}/provisioning/account_requests/${safeArId}/confirm`,
    {
      method: 'POST',
      headers,
    }
