...
} from 'data/partners/company-product'
...
const CompanyProductLoginPage: NextPageWithLayout = () => {
        ...
        {state === 'approval' && accountRequest && (
          <>
            <h2 className="py-2 text-lg font-medium">
              {PARTNER_COMPANY_NAME} {PARTNER_PRODUCT_NAME} Account Request
            </h2>
            <p className="text-center text-foreground-light">
              {PARTNER_COMPANY_NAME} {PARTNER_PRODUCT_NAME} wants to create a Supabase organization
              for <strong>{accountRequest.email}</strong>
            ...
            <p className="text-sm text-foreground-lighter text-center">
              This will create a new Supabase organization linked to your {PARTNER_COMPANY_NAME}{' '}
              account.
            </p>
          </>
        )}
...
export default withAuth(CompanyProductLoginPage)
