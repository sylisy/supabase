...
Postgres has a native `enum` type, but it has a significant limitation: you can add new values but you can't remove or rename existing ones without recreating the entire type. For most applications, a `text` column with a **check constraint** is simpler and more flexible:
