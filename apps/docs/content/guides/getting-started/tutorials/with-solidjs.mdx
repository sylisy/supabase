...
And finally, save the environment variables in a `.env`, you need the API URL and the key that you copied [earlier](#get-api-details).
...
Set up a SolidJS component to manage logins and sign ups which uses Magic Links, so users can sign in with their email without using passwords.
