...
All projects on Postgres `15.8.1.079` and newer will be using the newer physical backup process.

Projects on older Postgres versions will have to upgrade in order to be transitioned to physical backups. Once upgraded to an eligible version, then your project will automatically be transitioned over to physical backups.
